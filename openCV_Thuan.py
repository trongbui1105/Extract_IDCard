...
def crop_mssv(result):
    img_mssv = result[150:175, 150:270]
    return img_mssv
#Hàm cắt trường mssv

def crop_hoten_sinhvien(result):
    img_hoten_sinhvien = result[110:150, 150:400]
    return img_hoten_sinhvien
#Hàm cắt trường họ tên
...
def main():
    image = img_load('./Image/TheSinhVien/TheSinhVien2.jpg')
    bilateral = img_retouch(image)
    approx = find_contours(bilateral,image)
    result = crop_contours(approx, image)
    show_img(result)
    so_cmnd = crop_mssv(result)
    show_img(so_cmnd)
    ho_ten = crop_hoten_sinhvien(result)
    show_img(ho_ten)
