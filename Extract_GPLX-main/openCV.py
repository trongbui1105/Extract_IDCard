...
# def crop_gplx(result):
#     img_gplx = result[70:103, 225:390]
#     return img_gplx
def crop_gplx(result):
    img_gplx = result[65:91, 280:400]
    return img_gplx
#Hàm cắt trường gplx
...
#     return img_hoten
def crop_hoten(result):
    img_hoten = result[90:114, 230:430]
    return img_hoten
...
def main():
    image = img_load('./Image/gplx5.jpg')
    ...
    so_gplx = crop_gplx(result)
    show_img(so_gplx)

    ho_ten = crop_hoten(result)
    show_img(ho_ten)

    so_gplx_retouch = img_retouch(so_gplx)
    show_img(so_gplx_retouch)

    ho_ten_retouch = img_retouch(ho_ten)
    show_img(ho_ten_retouch)

    print('So gplx : ', char_recog(so_gplx_retouch))
